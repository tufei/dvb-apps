                                        ...
                                        uint32_t max_sessions)
{
    ...
    // setup structure
    sl = (struct en50221_session_layer*) malloc(sizeof(struct en50221_session_layer));
    if (sl == NULL)
        goto error_exit;
...
void en50221_sl_register_lookup_callback(struct en50221_session_layer *sl, en50221_sl_lookup_callback callback, void *arg)
{
...
int en50221_sl_create_session(struct en50221_session_layer *sl, int slot_id, uint8_t connection_id, uint32_t resource_id,
                              en50221_sl_resource_callback callback, void* arg)
{
    // lookup next free session_id:
    pthread_mutex_lock(&sl->global_lock);
    int session_number = en50221_sl_alloc_new_session(sl, resource_id, slot_id, connection_id, callback, arg);
    if (session_number == -1) {
...
int en50221_sl_destroy_session(struct en50221_session_layer *sl, uint16_t session_number)
{
...
int en50221_sl_send_data(struct en50221_session_layer *sl, uint16_t session_number, uint8_t *data, uint16_t data_length)
{
    if (session_number >= sl->max_sessions) {
        sl->error = EN50221ERR_BADSESSIONNUMBER;
        return -1;
...
int en50221_sl_send_datav(struct en50221_session_layer *sl, uint16_t session_number,
...
int en50221_sl_broadcast_data(struct en50221_session_layer *sl, int slot_id, uint32_t resource_id,
                              uint8_t *data, uint16_t data_length)
{
    ...
    for(i = 0; i < sl->max_sessions; i++)
    {
        ...
        if ((slot_id != -1) && (slot_id != sl->sessions[i].slot_id)) {
            pthread_mutex_unlock(&sl->sessions[i].session_lock);
            continue;
        }
                                     ...
                                     uint8_t *data, uint32_t data_length, uint8_t slot_id, uint8_t connection_id)
{
    ...
    if (status == S_STATUS_OPEN) {
        // lookup next free session_id:
        pthread_mutex_lock(&sl->global_lock);
        session_number = en50221_sl_alloc_new_session(sl, connected_resource_id, slot_id, connection_id,
...
static void en50221_sl_transport_callback(void *arg, int reason, uint8_t *data, uint32_t data_length,
                                          uint8_t slot_id, uint8_t connection_id)
{
    struct en50221_session_layer *sl = (struct en50221_session_layer *) arg;
    uint32_t i;
    ...
    switch(spdu_tag)
    {
        case ST_OPEN_SESSION_REQ:
            en50221_sl_handle_open_session_request(sl, data+1, data_length-1, slot_id, connection_id);
            break;

        case ST_CLOSE_SESSION_REQ:
            en50221_sl_handle_close_session_request(sl, data+1, data_length-1, slot_id, connection_id);
            break;

        case ST_SESSION_NUMBER:
            en50221_sl_handle_session_package(sl, data+1, data_length-1, slot_id, connection_id);
            break;

        case ST_CREATE_SESSION_RES:
            en50221_sl_handle_create_session_response(sl, data+1, data_length-1, slot_id, connection_id);
            break;

        case ST_CLOSE_SESSION_RES:
            en50221_sl_handle_close_session_response(sl, data+1, data_length-1, slot_id, connection_id);
            break;
