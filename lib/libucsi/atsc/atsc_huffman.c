...
static inline int huffbuff_bits(struct huffbuff *hbuf, uint8_t nbits)
{
  ...
  while(nbits--) {
    ...
    result <<= 1;
    if (hbuf->buf[hbuf->cur_byte] & (1 << hbuf->cur_bit))
      result |= 1;
                          ...
                          struct hufftree_entry hufftree[][128])
{
  ...
  while(hbuf.cur_byte < hbuf.buf_len) {
    ...
    if (treeval & HUFFTREE_LITERAL_MASK) {
      ...
      case HUFFSTRING_ESCAPE:
        if ((tmp = huffman_decode_uncompressed(&hbuf, dest, destlen, &destpos)) < 0)
          return tmp;
        if (tmp == 0)
          return destpos;
      ...
      default:
        ...
        // stash it
	(*dest)[destpos++] = treeval & ~HUFFTREE_LITERAL_MASK;
...
extern int atsc_text_decode_program_title_segment(uint8_t *buf, size_t buflen,
					     uint8_t **dest, size_t *destlen)
{
	return huffman_decode(buf, buflen, dest, destlen, program_title_hufftree);
}

extern int atsc_text_decode_program_description_segment(uint8_t *buf, size_t buflen,
		uint8_t **dest, size_t *destlen)
{
	return huffman_decode(buf, buflen, dest, destlen, program_description_hufftree);
}
