...
static int zap_data_create_decoder_filter(int adapter, int demux, uint16_t pid, int pestype);
static int zap_data_create_dvr_filter(int adapter, int demux, uint16_t pid);

static void zap_data_decoder_pmt(struct mpeg_pmt_section *pmt);
static void zap_data_dvr_pmt(struct mpeg_pmt_section *pmt);

static void zap_data_append_pid_fd(int pid, int fd);
static void zap_data_free_pid_fds(void);
...
void zap_data_start(int _output_type,
		    ...
		    char* outif, struct sockaddr_in outaddr, int _usertp)
{
	...
	case OUTPUT_TYPE_FILE:
	case OUTPUT_TYPE_UDP:
		pat_fd_dvrout = zap_data_create_dvr_filter(adapter_id, demux_id, TRANSPORT_PAT_PID);
	}
}
...
void zap_data_new_pat(int pmt_pid)
{
	...
	case OUTPUT_TYPE_UDP:
		if (pmt_fd_dvrout != -1)
			close(pmt_fd_dvrout);
		pmt_fd_dvrout = zap_data_create_dvr_filter(adapter_id, demux_id, pmt_pid);
	}
}

int zap_data_new_pmt(struct mpeg_pmt_section *pmt)
{
	...
	case OUTPUT_TYPE_DECODER:
	case OUTPUT_TYPE_DECODER_ABYPASS:
		zap_data_decoder_pmt(pmt);
		break;

	case OUTPUT_TYPE_DVR:
	case OUTPUT_TYPE_FILE:
	case OUTPUT_TYPE_UDP:
		zap_data_dvr_pmt(pmt);
		break;
	}
...
static void zap_data_decoder_pmt(struct mpeg_pmt_section *pmt)
{
	...
	if (audio_pid != -1) {
		int fd = zap_data_create_decoder_filter(adapter_id, demux_id, audio_pid, DVBDEMUX_PESTYPE_AUDIO);
		if (fd < 0) {
			fprintf(stderr, "Unable to create dvr filter for PID %i\n", audio_pid);
		} else {
			zap_data_append_pid_fd(audio_pid, fd);
		}
	}
	if (video_pid != -1) {
		int fd = zap_data_create_decoder_filter(adapter_id, demux_id, audio_pid, DVBDEMUX_PESTYPE_VIDEO);
		if (fd < 0) {
			fprintf(stderr, "Unable to create dvr filter for PID %i\n", audio_pid);
		} else {
			zap_data_append_pid_fd(audio_pid, fd);
	...
	int fd = zap_data_create_decoder_filter(adapter_id, demux_id, pmt->pcr_pid, DVBDEMUX_PESTYPE_PCR);
	if (fd < 0) {
		fprintf(stderr, "Unable to create dvr filter for PID %i\n", pmt->pcr_pid);
	} else {
		zap_data_append_pid_fd(pmt->pcr_pid, fd);
	}
}
...
static void zap_data_dvr_pmt(struct mpeg_pmt_section *pmt)
{
	struct mpeg_pmt_stream *cur_stream;
	mpeg_pmt_section_streams_for_each(pmt, cur_stream) {
		int fd = zap_data_create_dvr_filter(adapter_id, demux_id, cur_stream->pid);
		if (fd < 0) {
			fprintf(stderr, "Unable to create dvr filter for PID %i\n", cur_stream->pid);
		} else {
			zap_data_append_pid_fd(cur_stream->pid, fd);
		}
	}
}
...
static void zap_data_free_pid_fds()
{
	if (pid_fds_count) {
		int i;
